...
def Usage(as_script):
    if as_script:
        ...
        sys.stdout.write(
            "\t"
            + sys.argv[0]
            + "<outputdir name> <fileprefix=brscan> <time in seconds from epoch>"
            " <etc>\n"
        )
        sys.stdout.flush()
...
def files_within_timeoffset(
    l, match_string_time, match_string_part, timenow, timeoffset, debug=False
):
    ...
    if len(matches) > 0:
        ...
        matches.sort(
            reverse=True
        )  # will sort by first integer key, by default. it appears to work correctly
...
def get_default_device():
    ...
    cmd = subprocess.Popen(
        ["scanimage", "-L"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    try:
        ...
        print(
            "No brother device found by script: here is scanimage -L output: " + out,
            file=sys.stderr,
        )
        return None
...
def get_default_duplex_source(device_name):
    ...
    if re.findall(r"--source", out):
        try:
            if debug:
                logprint(
                    "get_default_duplex_source: Found source line in scanimage output."
                )
        except:
...
def run_scancommand(
    ...
    if not dry_run:
        run = subprocess.Popen(
            scancommand, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
